...
function HashSet() {
    ...
    buckets.forEach((el) => {
      ...
      while (tempNode !== null) {
        ...
        if (
          ...
          doubleSizeBuckets[hashCode] = createList;
          doubleSizeBuckets[hashCode].prepend({ key });
        } else if (
          doubleSizeBuckets[hashCode] !== undefined ||
          doubleSizeBuckets[hashCode] !== null
        ) {
          ...
          doubleSizeBuckets[hashCode].append({ key });
        }
  ...
  const entries = () => {
    ...
    buckets.forEach((el) => {
      const logKey = el.logKeyLL();

      allHashMapPairKeyValue.push(logKey);
    });
    const flatAllHashMapPairKeyValue = allHashMapPairKeyValue.flat();
